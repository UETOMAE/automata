...
def get_data_from_browserstack(path):
    req = urllib2.Request('https://api.browserstack.com/%s' % path)
...
builds_info = get_data_from_browserstack('/automate/builds.json')
...
for build_info in builds_info:
    ...
    sessions_info = get_data_from_browserstack('/automate/builds/%s/sessions.json' % build_id)
